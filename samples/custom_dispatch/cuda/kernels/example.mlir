...
// selection.
#nvptx_target = #hal.executable.target<"cuda", "cuda-nvptx-fb", {target_arch = "sm_52"}>
...
#cuda_target = #hal.device.target<"cuda", {
  executable_targets = [#nvptx_target],
...
module @example attributes {hal.device.targets = [#cuda_target]} {
  ...
  hal.executable private @kernels {

    // Externally-defined variant in PTX format.
    hal.executable.variant public @nvptx, target = #nvptx_target, objects = [
      // Object files linked into the executable.
      // Certain backends (today) support either wholesale definition or linking
      // of partial objects for imports used by generated code.
      #hal.executable.object<{
        // Referencing a file path on disk but could also have the data embedded
        // in order to make the MLIR file hermetic/portable across compilation
        // pipelines. In the future we'll likely use MLIR's external resource
        // functionality for this.
        path = "samples/custom_dispatch/cuda/kernels/kernels.ptx"
      }>
    ] {
  ...
  func.func @mixed_invocation(%arg0: tensor<?xf32>, %arg1: tensor<?xf32>) -> tensor<?xf32> {
    ...
    // when multi-targeting (@nvptx will be chosen if available).
    %0 = flow.dispatch @kernels::@simple_mul[%dim](%dim_i32, %arg0, %arg1) {
    ...
    // to allow user-controlled overrides of the kernels, custom selection logic
    // based on runtime parameters, etc. In general, though, the above automatic
    ...
    %2 = flow.dispatch @kernels::@nvptx::@simple_mul_inplace[%dim](%dim_i32, %0, %1) : (i32, tensor<?xf32>{%dim}, tensor<?xf32>{%dim}) -> %1{%dim}
