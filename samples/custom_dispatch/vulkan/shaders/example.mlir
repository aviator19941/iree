...
module @example attributes {hal.device.targets = [#vulkan_target]} {
  ...
  hal.executable private @simple_mul {
    ...
    // Externally-defined variant in SPIR-V bytecode format.
    hal.executable.variant public @spirv, target = #spirv_target, objects = [
  ...
  hal.executable private @simple_mul_inplace {
    hal.executable.variant public @spirv, target = #spirv_target, objects = [
      #hal.executable.object<{
  ...
  func.func @mixed_invocation(%arg0: tensor<?xf32>, %arg1: tensor<?xf32>) -> tensor<?xf32> {
    ...
    // when multi-targeting (@spirv will be chosen if available).
    ...
    // to allow user-controlled overrides of the shaders, custom selection logic
    ...
    %2 = flow.dispatch @simple_mul_inplace::@spirv::@main[%dim](%dim_i32, %0, %1) : (i32, tensor<?xf32>{%dim}, tensor<?xf32>{%dim}) -> %1{%dim}
